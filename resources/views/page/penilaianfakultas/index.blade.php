...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12 grid-margin">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h4 class="font-weight-bold mb-0 ml-5">Data Penilaian Fakultas</h4>
                </div>
            </div>
        </div>
    </div>
                ...
                <h4 class="card-title">
                    @if (Auth::user()->role == 'juri')
                        <button type="button" class="btn btn-primary btnTambahData"
                            onclick="window.location='{{ url('penilaian-fakultas/add') }}'">
                            <i class="fas fa-plus"></i> Tambah Data
                        </button>
                    @elseif (Auth::user()->role == 'admin')
                        <button type="button" class="btn btn-success"
                            onclick="window.location='{{ url('penilaian-fakultas/doExportExcel') }}'">
    ...
    <script>
        function editdata(id) {
            window.location.href = ("{{ url('penilaian-fakultas/edit') }}" + '/' + id);
        }

        function removedata(id) {
            Swal.fire({
                title: 'Hapus Penilaian',
                text: "Yakin menghapus data penilaian fakultas ini ?",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: "{{ url('penilaian-fakultas/deleteData') }}" + '/' + id,
                        dataType: "json",
                        beforeSend: function() {
                            $('#deleteIcon_' + id).hide();
        ...
        $(document).ready(function() {
            ...
            const table = $('#datapenilaian').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ url('penilaian-fakultas/index') }}",
